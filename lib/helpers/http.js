...
module.exports = function(requestLib_optional, protocol, host, port, rootPath, headers_optional, strictSSL) {
    ...
    function createRequestOptions(method, path, body, headers, callback) {
        ...
        if(body) {
            assert.ok(typeof body === "object" || typeof body === "string" || Buffer.isBuffer(body), "Body should be of type String, object or be a buffer (got " + typeof body + ")");
            body = evaluate(body);
            if(Buffer.isBuffer(body)) {
                ...
                if(!headers.hasOwnProperty('content-length')) {
                    var length = typeof body === "object" ? Buffer.byteLength(JSON.stringify(body)) : Buffer.byteLength(body);
