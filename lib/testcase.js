...
function runTest(test, done) {
	var d = domain.create();
	if(this.overallTimeout === false) {
		this.overallTimeout = this.tests.length * this.timeout;
	}
	d.on('error', done);

	var testTimerId = setTimeout(function() {
		d.dispose();
		return done(new Error('callback was not called in the test, timed out after ' + this.overallTimeout + ' ms'));
	}.bind(this), this.overallTimeout);

	//run the test itself in a save domain
	d.run(function() {
